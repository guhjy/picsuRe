...
nicer.result <- function(result, allpaths, verbose = FALSE)  {
  ...
  for (e in 1:(length(groups2)))  {
    subdf <- result[-1][groups == groups2[e]]
    if (length(unique(subdf[,1])) <= 2  & any(is.na(unique(subdf[,1]))))  {
      ...
      if (!all(is.null(names(groups2)))) {
          if (names(groups2[e]) != "") {
            cnames <- c(cnames, names(groups2[e]))
          } else {
      ...
      if (!is.null(names(groups[groups == groups2[e]])))  {
        cnames <- c(cnames, names(groups[groups == groups2[e]]))
      } else {
